...
std::string stateToText(uint32_t state) {
    ...
    oss << "Unbekannter State (0x" << std::hex << std::uppercase << state << ")";
    return oss.str();
}
...
int main(int argc, char* argv[]) {
    ...
    std::string logfile = "/home/pi/__ctrl_minimal/data/eta" + std::to_string(node_id) + "_sdo_state_log.txt";
    ...
    log << "\n--- ETA" << node_id << " State-Abfrage am " << nowTimeString() << " ---\n";
    std::cout << "\n--- ETA" << node_id << " State-Abfrage am " << nowTimeString() << " ---\n";
    ...
    if (readSDO(s, node_id, index, subindex, value, resp_code)) {
        log << "0x" << std::hex << std::setw(4) << std::setfill('0') << value
        << " = " << stateToText(value) << std::endl;
