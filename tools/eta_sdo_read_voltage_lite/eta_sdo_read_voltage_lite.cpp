...
// Eintrag in SQLite machen
void insertVoltageToDB(const std::string &dbfile, const std::string &type, int node_id,
                       float voltage, int voltage_raw, int index, int subindex)
{
    sqlite3 *db;
    char *errMsg = nullptr;
    ...
    oss << "INSERT INTO voltage_log (type, node_id, voltage, voltage_raw, od_index, od_subindex, timestamp) VALUES ("
        << "'" << type << "',"
        << node_id << ","
        << voltage << ","
        << voltage_raw << ","
...
int main(int argc, char *argv[])
{
    ...
    uint16_t index = 0x6040; // act. voltage
    ...
    {
        float voltage = value * 0.001f;
        std::cout << "act. voltage: " << voltage << " V (raw: " << value << ")" << std::endl;
        insertVoltageToDB(dbfile, type, node_id, voltage, value, index, subindex);
